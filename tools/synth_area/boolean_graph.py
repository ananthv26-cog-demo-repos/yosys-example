...
the caller asked for a graph of supported gates only, so nothing is approximated.

Edges are (driver node -> sink node, sink pin). `max_depth` is the longest chain of
combinational gates between graph sources (inputs, constants, DFF outputs) and sinks
(outputs, DFF inputs); `max_fanout` / `avg_fanout` count sink pins per driving node.
"""
    ...
    for wname, w in sorted(mod.get("netnames", {}).items(), key=lambda kv: (kv[1].get("hide_name", 0), kv[0])):
        for i, b in enumerate(w.get("bits", [])):
            if isinstance(b, int) and b not in bit_name:
                bit_name[b] = wname if len(w["bits"]) == 1 else f"{wname}[{i}]"

    ports = mod.get("ports", {})
    for pname, p in ports.items():
        for i, b in enumerate(p.get("bits", [])):
            label = pname if len(p["bits"]) == 1 else f"{pname}[{i}]"
            if p["direction"] in ("input", "inout"):
                nid = new_node("INPUT", name=label, port=pname, bit=i)
                if isinstance(b, int):
                    driver_of.setdefault(b, nid)
            if p["direction"] in ("output", "inout"):
                new_node("OUTPUT", name=label, port=pname, bit=i, source_bit=b)
...
def compute_metrics(graph: dict) -> dict:
    ...
    clock_edges = [e for e in edges if id2node[e["to"]]["kind"] == "DFF" and e["pin"] == "C"]
    data_edges = [e for e in edges if not (id2node[e["to"]]["kind"] == "DFF" and e["pin"] == "C")]
    ...
    for nid in order:
        ...
        if n["kind"] == "GATE":
            ...
            depth[nid] = 0
    sink_depths = [
        max((depth.get(d, 0) for d in n["inputs"].values()), default=0)
        for n in nodes
        if n["kind"] in ("OUTPUT", "DFF")
    ]
    gate_depths = [depth[nid] for nid in depth if nodes[nid]["kind"] == "GATE"]
    max_depth = max(gate_depths + sink_depths, default=0)

    dff_fanins = Counter()
    for n in nodes:
        if n["kind"] == "DFF":
            dff_fanins.update(n["inputs"].keys())
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        graph = build_graph(json.loads(Path(args.yosys_json).read_text()), args.top)
